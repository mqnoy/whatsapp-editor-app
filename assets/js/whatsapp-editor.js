(function ($) {
    ...
    $.fn.whatsappEditor = function (options) {
        ...
        function initToolbar($parentElement) {
            ...
            if (settings.bold) {
                $("<a/>")
                    ...
                    .append("<span class='fa fa-bold fa-fw' title='bold'></span>")
                    .on("click", function (e) {
                        format("bold");
                    })
                    .appendTo($toolbar);
            ...
            if (settings.italic) {
                $("<a/>")
                    .attr("href", "javascript:void(0)")
                    .append("<span class='fa fa-italic fa-fw' title='italics'></span>")
                    .on("click", function (e) {
                        format("italic");
                    })
                    .appendTo($toolbar);
            ...
            if (settings.strikethrough) {
                $("<a/>")
                    .attr("href", "javascript:void(0)")
                    .append("<span class='fa fa-strikethrough fa-fw' title='strikethrough'></span>")
                    .on("click", function (e) {
            ...
            if (settings.monospace) {
                $("<a/>")
                    .attr("href", "javascript:void(0)")
                    .append("<span class='fa fa-text-width fa-fw' title='monospace'></span>")
                    .on("click", function (e) {
                        format("monospace");
                    })
                    .appendTo($toolbar);
            ...
            if (settings.clipboard) {
                $("<a/>")
                    .attr("href", "javascript:void(0)")
                    .append("<span class='fa fa-bold fa-clipboard' title='copy to clipboard'></span>")
                    .on("click", function (e) {
                        actionToCopy($parentElement)
                    })
                    .appendTo($toolbar);
            }
        ...
        function actionToCopy($parentElement) {
            const value = getWhatsappFormatedMsg($parentElement)

            let copyElement = document.createElement('input');
            copyElement.setAttribute('type', 'text');
            copyElement.setAttribute('value', value);
        ...
        function getWhatsappFormatedMsg($parentElement) {
            const pe = $($parentElement);
            const waEditor = pe.find('.whatsapp-editor')
            const raw = prepareFormattedContent(waEditor.html())
            const parsed = raw
                .replace(/<\/div>/g, '')
                .replace(/<div>|<br>|<br\/>/g, '\\n');
